	...
	public void Run(Commands commands) {
		...
		foreach (var (location, movement) in query) {
			if (movement.currentTarget != null && movement.path.Count == 0) {
				...
				GD.PrintS("(PathfindingSystem) From:", location.hex, "To:", movement.currentTarget);
				var path = pathfinder.getPath(fromTile, toTile);
				if (path == null) {
					GD.PrintS("(PathfindingSystem) No path to target, removing target");
					movement.currentTarget = null;
					movement.movementAction.status = ActionStatus.Cancelled;
				} else {
					GD.PrintS("(PathfindingSystem) Found path:", String.Join(", ", path));
